...
const errorHandler = (error) => {
    ...
    if (error.response && error.response.status === 401) {
        ...
        // axios.post(
        //     Constants.BASE_ENDPOINT + 'auth/login',
        ...
        Alert.error({title: error.response.data && error.response.data.data && error.response.data.data.message && typeof error.response.data.data.message === 'string' ? error.response.data.data.message : 'something went wrong', status: 406})
        return error
    } else if (error.response && error.response.status === 400) {
        Alert.error({title: 'something went wrong', status: 400});
        return error
    } else if (error.response && error.response.status === 422) {
        Alert.error({title: error.response.data && error.response.data.data && error.response.data.data.message && typeof error.response.data.data.message === 'string' ? error.response.data.data.message : 'something went wrong', status: 422})
        return error
    } else {
