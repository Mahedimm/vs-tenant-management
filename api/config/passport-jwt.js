...
const {UserModel: landlordUserModel} = require("../models/landlordUser.model");
...
const passportJwtInit = new passportJwt.Strategy({
    ...
    if (jwt_payload.platform === 'tenant')
        user = await landlordUserModel.findOne({_id: jwt_payload.sub._id});
    else if (jwt_payload.platform === 'landlord')
        user = await landlordUserModel.findOne({_id: jwt_payload.sub._id});
    else
        return done(true, false);
