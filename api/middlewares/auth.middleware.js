...
const {OAuthAccessTokenModel: landlordOAuthAccessTokenModel} = require("../models/landlordOAuthAccessToken.model");
const {OAuthRefreshTokenModel: landlordOAuthRefreshTokenModel} = require("../models/landlordOAuthRefreshToken.model");
...
const verifyCallback = (req, resolve, reject) => async (err, user, info) => {
    ...
    if (info.platform) {
        if (info.platform === 'tenant') {
        ...
        } else if (info.platform === 'landlord') {
            const oAuthTokenDetail = await landlordOAuthAccessTokenModel.findOne({
                _id: req.headers.authorization.split(" ")[1],
                revoked: false,
                expires: {$gte: moment().format()}
            });
            ...
            if (oAuthTokenDetail) {
                const oAuthRefreshDetail = await landlordOAuthRefreshTokenModel.findOne({
                    accessToken: oAuthTokenDetail._id,
                    revoked: false,
                    expires: {$gte: moment().format()}
                });
